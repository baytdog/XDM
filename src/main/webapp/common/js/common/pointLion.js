...
//		    }
		...
		var selectManyUser1 = function(paraStr,oldData,callback){
			if(!paraStr){paraStr="";}
			if(!oldData){oldData="";}
			layer.open({
			  type: 2,
			  ...
			  btn: ['添加所选', '确定'], 
			  btnAlign: 'c',
			  content: ctx+'/admin/sys/user/openSelectManyUserPage1?'+paraStr+'&oldData='+oldData,
			  success: function(layero){
				  selectManyUserIframe = window[layero.find('iframe')[0]['name']];
			  },
			  yes: function(){
		...
		return {
			...
			selectManyUser1:selectManyUser1,//选择多个人
