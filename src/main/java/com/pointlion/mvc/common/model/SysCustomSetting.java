...
import com.pointlion.mvc.common.model.base.BaseSysCustomSetting;
...
public class SysCustomSetting extends BaseSysCustomSetting<SysCustomSetting> {
	...
	public SysCustomSetting getCstmSettingByUsername(String userid){
		...
		if(findFirst.getType().equals("3")) {
			return this.dao().findFirst("select * from sys_custom_setting s where s.user_id='moa'");
		}
	...
	public SysCustomSetting getLoginUserCstSetting(){
		OaTypes findFirst = OaTypes.dao.findFirst("select  * from oa_types where  status='1'");
		if(findFirst.getType().equals("3")) {
			//return SysCustomSetting.dao.getCstmSettingByUsername("moa");
		}
