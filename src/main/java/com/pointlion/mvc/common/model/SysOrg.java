...
public class SysOrg extends BaseSysOrg<SysOrg> {
	...
	public List<SysOrg> getChildrenByPid1(String id,String type){
		String sql = "select * from sys_org m where m.parent_id='"+id+"' and id !='ef7e017b3e5642f58304d230ecbad672'";
		if(StrKit.notBlank(type)){
			sql = sql + " and m.type='"+type+"' ";
		}
		sql = sql + " order by m.sort";
	 ...
	 * @return
	 */
	public List<SysOrg> getChildrenAllTree1(String id){
		List<SysOrg> list =  getChildrenByPid1(id,null);//根据id查询孩子
		for(SysOrg o : list){
			o.setChildren(getChildrenAllTree(o.getId()));
		}
		return list;
	...
	public SysOrg getFirstChildCompany(SysOrg org){
		...
		if(allList!=null&&allList.size()>0){
			for(SysOrg o:allList){
				if(StrKit.isBlank(o.getParentChildCompanyId())&&!Constants.SYS_MENU_ROOT.equals(o.getId())&&Constants.SYS_MENU_ROOT.equals(o.getParentId())){
					result = o;
				}
			}
		}
