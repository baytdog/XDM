...
public class XdAttendanceController extends BaseController {
	public static final XdAttendanceService service = XdAttendanceService.me;
     ...
     */
    public void save(){
    	XdAttendance o = getModel(XdAttendance.class);
		XdAttendance attendance = XdAttendance.dao.findById(o.getId());
    ...
    public void getEditPage(){
		...
		setAttr("view", view);
		XdAttendance o = new XdAttendance();
		...
		setAttr("o", o);
		setAttr("formModelName",StringUtil.toLowerCaseFirstOne(XdAttendance.class.getSimpleName()));
    ...
    /**
     * @Method getHomeAttendanceListPage
     * @Date 2023/1/14 16:46
     * @Description 获取出勤首页列表
     * @Author king
     * @Version  1.0
     * @Return void
     */
	public void getHomeAttendanceListPage(){
		renderIframe("homeList.html");
	}
	/**
	 * @Method getHomeAttendanceList
	 * @Date 2023/1/14 16:51
	 * @Description 获取首页提醒列表数据
	 * @Author king
	 * @Version  1.0
	 * @Return void
	 */
	public void getHomeAttendanceList(){
		String curr = getPara("pageNumber");
		String pageSize = getPara("pageSize");
	...
	/**
	 * @Method openHomeAttendancePage
	 * @Date 2023/1/14 17:04
	 * @Description 首页提醒打开表单
	 * @Author king
	 * @Version  1.0
	 * @Return void
	 */
	public void openHomeAttendancePage(){
		...
		XdAttendance o = XdAttendance.dao.findById(oid);
		o.getCreateUser();
		...
		setAttr("formModelName",StringUtil.toLowerCaseFirstOne(XdAttendance.class.getSimpleName()));//模型名称
	...
	public void apprPass(){
		...
		if(status.equals("0")){
			attendance.setStatus("1");//部门领导审批通过
		}else if(status.equals("1")){
			attendance.setStatus("2");//人事部审批通过
		}
		DateTimeFormatter dtf =  DateTimeFormatter.ofPattern("yyyy-MM-dd");
		String format = LocalDate.now().format(dtf);
		String remarks=attendance.getRemarks()==null?"":attendance.getRemarks();
	...
	public void apprrNoPass(){
		...
		if(status.equals("0")){
			attendance.setStatus("3");//部门领导审批不通过
		}else if(status.equals("1")){
			attendance.setStatus("4");//人事部审批不通过
		}
	...
	public void sendAppr(){
		...
		nextSteps.setOid(oid);
		nextSteps.setStype(STYPE);
		String approverId = attendance.getApproverId();
