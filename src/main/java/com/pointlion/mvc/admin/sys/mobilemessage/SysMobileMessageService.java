...
//	}
	...
	public void sendMeesageByEXin(List<String> mobileList,String toUserid,String toUsername,String toName,String message){
		...
		formParams.add(new BasicNameValuePair("SerialNumber", StringUtil.addZeroForNum(DateUtil.format(new Date(), 21), 20,"right")));//流水号，20位数字，每个请求流水号要求唯一（规则自定义,建议时间格式精确到毫秒）必填参数，与回执接口中的流水号一一对应，不传后面回执接口无法查询数据。
